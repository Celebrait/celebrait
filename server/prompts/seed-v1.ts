...
TYPOGRAPHY INTEGRATION (CRITICAL — TEXT MUST BE LARGE AND DOMINANT): The card text is the single most important visual element after the character. It MUST occupy at least 25-30% of the image area and be instantly readable at arm's length. Place text in the lower third or across a clear area of the composition. The text should be LARGE, BOLD, and visually striking — think billboard-scale lettering, not subtle engraving. Integration methods: written in sand with deep grooves, painted on a wall in large brushstrokes, displayed on a prominent sign or banner, formed by large natural elements (clouds, flowers, lights), or rendered as large decorative typography that fills a significant portion of the frame. NEVER hide text in small details, carve it tiny into surfaces, or embed it subtly into architecture. The text must POP — a viewer should read it within the first second of looking at the card.{{#if includeText}}. STRICT TEXT RESTRICTION: Add EXACTLY and ONLY the text "{{cardText}}" - ABSOLUTELY NO OTHER TEXT, WORDS, LETTERS, NUMBERS, SIGNS, LABELS, OR WRITING of any kind should appear anywhere in the image. CRITICAL: Do NOT overlay text on top of the image. Instead, naturally integrate ONLY this specific text into the scene as part of the artistic composition. FORBIDDEN: Do not add any background text, signage, labels, captions, watermarks, logos, brand names, location names, or any other written content. Typography should match the chosen artistic style and complement the scene's natural elements. CRITICAL: All letters of the text should be legible within the square frame of the artwork. CRITICAL: DO NOT allow the text to be cropped off the screen in any way; all letters in the text MUST be readable with no cropping of the text whatsoever.{{/if}}. High-quality artistic rendering, professional artwork.`;
