...
export function generateSaveProgressEmail(userData: any, host?: string): EmailParams {
  const baseUrl = host || `https://${process.env.REPLIT_DOMAINS?.split(',')[0] || 'localhost:5000'}`;
  const continueUrl = `${baseUrl}/saved-progress`;
  const recipientName = userData.recipientName || 'your recipient';
  const celebration = userData.celebration || 'greeting card';
  
  return {
    to: userData.email,
    from: 'greetings@celebrait.co.za',
    subject: `Continue Creating Your ${celebration} Card`,
    html: `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #f9fafb;">
      <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 30px; border-radius: 12px; text-align: center; margin-bottom: 30px;">
        <h1 style="color: white; margin: 0; font-size: 28px; font-weight: bold;">Continue Your Card Creation</h1>
        <p style="color: #e0e7ff; margin: 10px 0 0 0; font-size: 16px;">Your ${celebration} card is waiting for you</p>
      </div>
      ...
      <div style="background: white; padding: 30px; border-radius: 12px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); margin-bottom: 30px;">
        ...
        <p style="font-size: 16px; color: #374151; line-height: 1.6; margin-bottom: 20px;">
          You started creating a personalized ${celebration} card for ${recipientName}, and we've saved your progress so you can continue where you left off.
        </p>
        
        <p style="font-size: 16px; color: #374151; line-height: 1.6; margin-bottom: 30px;">
          Your card creation is partially complete and waiting for you to finish. All your answers and choices have been saved.
        </p>
        
        <div style="text-align: center; margin: 30px 0;">
          <a href="${continueUrl}" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 15px 30px; text-decoration: none; border-radius: 8px; font-weight: bold; font-size: 16px; display: inline-block; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
            Continue Creating Your Card
          </a>
        </div>
        
        <p style="font-size: 14px; color: #6b7280; line-height: 1.6; margin-top: 30px;">
          Don't worry - your progress is securely saved and you can return anytime to complete your personalized card.
        </p>
      </div>
      
      <div style="text-align: center; color: #6b7280; font-size: 14px;">
        <p>This link will take you directly to your saved progress.</p>
        <p>© 2025 Celebrait. Create personalized greeting cards with AI.</p>
      </div>
    </div>
    `,
    text: `
    Continue Creating Your ${celebration} Card
    ...
    Continue creating your card: ${continueUrl}
    
    Don't worry - your progress is securely saved and you can return anytime to complete your personalized card.
    
    © 2025 Celebrait. Create personalized greeting cards with AI.
    `
  };
}

export async function sendSaveProgressEmail(userData: any, host?: string): Promise<boolean> {
  if (!hasValidSendGridConfig()) {
    console.log('SendGrid not configured, skipping save progress email');
    return false;
  }

  try {
    const emailParams = generateSaveProgressEmail(userData, host);
    return await sendEmail(emailParams);
  } catch (error) {
    console.error('Error sending save progress email:', error);
    return false;
  }
}