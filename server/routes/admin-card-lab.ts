...
export function v2SystemPrompt(visual: 'celebrait' | 'open' | 'charm', slots: Array<{ angle: string; format?: string; register: string; territory?: string; ground?: string; tone?: string; cast?: boolean }>, occasionBrief: string): string {
...
⚠️ Your previous rewrite still contained protected material — remove it completely this time.` : ''}` },
  ...
  const renderHandler = async (req: Request, res: Response) => {
    ...
    const prompt = [
      ...
      body.cameoPhoto ? `⚠️ THE CAMEO — THE PEOPLE IN THE REFERENCE PHOTO ARE THE CARD'S STARS, PAINTED INTO ITS WORLD AS FULL PARTICIPANTS. THE FACE IS SACRED: each person's face is a faithful illustrated likeness — face shape and features, hairstyle and colour, skin tone, glasses or facial hair — instantly recognisable to someone who loves them, drawn in the card's own flat style, never photoreal. EVERYTHING BELOW THE CHIN BELONGS TO THE SCENE: dress them for this world, pose them genuinely taking part in it, give them the expression this card's mood calls for — their clothes and posture come from the card, not from the photo. EVERYONE in the photo appears; never drop, add or swap anyone. PORTRAIT SCALE, ALWAYS: faces big enough to recognise across a room — never a distant figure, never turned away, never a silhouette. A likeness nobody can see is a failed card.` : '',
