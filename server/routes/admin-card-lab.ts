...
⚠️ ONE SUBJECT MEANS ONE. A single object, a single place, a single arrangement — chosen well, drawn large, given room around it. The failure to fear is the tidy little collection: five small props laid out neatly, or a number assembled out of twenty tiny things. That card has stopped being designed and started being packed, and it is what makes a rack look busy and cheap from across a room. Restraint is the expensive-looking choice; the best card in any set is almost always the one with the fewest things on it. THE ORDER OF OPERATIONS IS BEAUTIFUL FIRST, JOKE SECOND: gorgeous at arm's length, then you notice the line — the card someone photographs and sends to the group chat. The register is a SPECTRUM you may use all of: from pure flat print to gently dimensional 3D-FLAT — paper-cut depth, soft shadows, rounded sculptural forms, tactile materials — as long as it stays graphic and deliberate. Photorealism is never home.
