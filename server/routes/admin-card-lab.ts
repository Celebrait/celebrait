...
STRIKING IS THE BAR. NOT LOUD. Those are different words and we mean the first. A card is striking when it commits: a confident ground, decisive contrast against it, and one colour clearly leading. A chalky pink card with one deep ink in it is striking. A card shouting in primary red and blue is merely loud, and a rack of those reads like a sports programme.
THE WHOLE MODERN RANGE IS OPEN AND IN CONSTANT ROTATION — dusty and chalky tones, soft brights, warm neutrals, deep jewel tones, faded and sun-bleached shades, rich darks, and yes the saturated poster colours as ONE option among many rather than the default. Soft palettes are fully welcome and must still be striking, which they earn through contrast and commitment rather than volume.
⚠️ THE OBSERVED FAILURE IS A RACK THAT DRIFTS ONE WAY. Ours drifted into primary poster colour with heavy industrial type and started to read narrow and blokey — a card everyone should be able to pick up looked like it was aimed at one sort of person. Judge it as a SHELF: a rack where every card shouts and a rack where none of them do are the same failure.
...
⚠️ GROUND tells you the WEIGHT of that card's background — soft (chalky, pale, sun-bleached), mid (a true colour at moderate depth) or deep (dark, rich, saturated). It does NOT tell you the hue: take that from the subject's colour world. The three are assigned deliberately different so the set has range on a shelf, and a set of three cards at the same weight is the drift we are fixing.
