...
` : ''}${interest?.trim() ? `- THE SUBJECT ANCHOR: the brief's subject is "${interest.trim()}". If that names a SPECIFIC show, film, book, team or place, at least one front must name it or unmistakably anchor it — a set answering a named subject with its generic CATEGORY (telly-in-general for a named sitcom, football-in-general for a named club) fails the set's whole reason for existing; flag every card that could be about the category rather than the subject. Judge anchored cards at a knowing buyer's half-second: the reference must be the subject's most famous tier and land instantly for someone who knows it — depth of fandom never excuses a second read.
