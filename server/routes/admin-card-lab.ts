...
const V2_CELEBRAIT_REGISTER = `THE LOOK — start every card from the Celebrait register: GRAPHIC and BOLD, one confident ground colour, type doing real work, objects not scenes, current not antique. THE ORDER OF OPERATIONS IS BEAUTIFUL FIRST, JOKE SECOND: gorgeous at arm's length, then you notice the line — the card someone photographs and sends to the group chat. The register is a SPECTRUM you may use all of: from pure flat print to gently dimensional 3D-FLAT — paper-cut depth, soft shadows, rounded sculptural forms, tactile materials — as long as it stays graphic and deliberate. Photorealism is never home.
...
⚠️ DO NOT LET THIS BRIEF COLLAPSE INTO A SINGLE IDEA. With nothing else asked of it, it seizes the first frame it thinks of and writes all three cards inside it — observed twice on the first day this ran: three cards of tasteful stationery, then three cards about the admin of being celebrated. Three cards, three genuinely different thoughts about the day, three different ground colours, three different treatments. One palette three times is one card printed three ways.`
        : interestText
