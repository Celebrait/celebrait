...
⚠️ WHO IS READING IT: the WORDS are usually read ALOUD by an adult, so they may carry a wink the grown-up enjoys — but the ARTWORK belongs entirely to the child and must delight THEM. Bright, saturated, generous; nothing muted, nothing editorial, nothing ironic.`
      : age <= 17
        ...
        : age <= 25
          ? `AGE BAND — THRESHOLD (${age}). The joke is CROSSING A LINE: doors opening, permissions arriving, being trusted with things you are gloriously unprepared for. Loud, energetic, meme-fluent. Bright saturated palette; bold playful type.
...
⚠️ NEVER AN AGEING JOKE. Nobody here is old, declining or past anything. Observed failure, "18. Proof Of Ageing" — a clean pun that resolves onto exactly the wrong idea. They are ARRIVING, and every line should know it.
...
1. WORDPLAY — the turn is in the LANGUAGE. A pun or a twist on a phrase from that world's own vocabulary. Must pass the pub test.
  ...
  ⚠️ AND THE SECOND MEANING HAS TO BE THE RIGHT ONE. A pun can resolve perfectly and still land on an idea that suits the brief badly — which is worse than a dead pun, because it reads as deliberate. Observed: "18. Proof Of Ageing" on an 18th. The wordplay is clean (proof of age, the ID they now get asked for) but it resolves onto AGEING, a decline joke, on the one birthday that is entirely about arriving. Say the second meaning out loud AND check it is something you would want on this card.
...
WRITE WIDE, THEN SHORTLIST: draft at least SIX candidate lines per angle, then be a ruthless editor. A line SURVIVES only if it passes ALL of:
   ...
   - ⚠️ THE BUYER TEST — most likely to lose us the sale. THE BUYER IS NOT THE FAN: a daughter buying for her Harry Potter dad may never have read a word. If she cannot tell at a glance it is good, she does not buy it. So use the FAMOUS layer that escaped the fandom — platform nine and three-quarters, "always", "I am your father" — never the deep cut (a minor character, an obscure spell, a squad number, an album track). Still specific, just specific in a way that carries beyond the fandom.
   - THE TURN TEST: name the surprise in five words or fewer — "puns on reel/real", "under-reacts to nine wasted hours". No nameable turn means DRY, and it dies here however warm or well written. Most weak lines fail here.
