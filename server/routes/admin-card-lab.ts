...
const V2_CELEBRAIT_REGISTER = `THE LOOK — start every card from the Celebrait register: GRAPHIC and BOLD, one confident ground colour, type doing real work, ONE SUBJECT PER CARD, current not antique.
...
⚠️ WHAT FAILS IS THE TOKEN CHECKLIST: unrelated props laid out flat to prove the theme, or a number assembled from twenty tiny things. Observed: a 25th for someone going to Ibiza came back as a boarding pass, a phone, sunglasses and a flight path, then again as a phone, a skyline, earbuds and an airport pint — four proofs of the topic, no picture. THE TEST: do the things on this card belong in the same photograph, or were they gathered to make a point? Belonging together is rich. Gathered to prove is packed. THE ORDER OF OPERATIONS IS BEAUTIFUL FIRST, JOKE SECOND: gorgeous at arm's length, then you notice the line — the card someone photographs and sends to the group chat. The register is a SPECTRUM you may use all of: from pure flat print to gently dimensional 3D-FLAT — paper-cut depth, soft shadows, rounded sculptural forms, tactile materials — as long as it stays graphic and deliberate. Photorealism is never home.
