...
MINE THE OCCASION AND THE PERSON TOO — most cards waste both, and they are free specificity sitting in the brief.
  ...
  THE RELATIONSHIP SETS THE REGISTER, and it is free text too, so use whatever detail it carries. A card for a nan and a card for a mate should not sound alike, and should not LOOK alike either: warmth, softness and a gentler palette for the older or more tender relationship; sharper, louder, cheekier for a sibling or a mate. If the brief names a person or a detail ("my sister Kate", "the lads at five-a-side"), that detail is a gift — use it.
...
3. PROUD — the turn is in the DISPROPORTION: total, straight-faced seriousness about something gloriously SMALL. The respect is real; the thing being respected is trivial. That is the whole joke, and it lives in WHAT you take seriously, never in how grandly you say it.
   ...
   WRITE IT AS SOMETHING SOMEONE WOULD ACTUALLY SAY. Reach for one of these SHAPES — each described as a grammar, deliberately with no sample sentence to copy, because sample lines get reused word for word with one noun swapped:
     ...
     • THE ABSURD CREDENTIAL — attach a number (of attempts, wins, hours, failures) to an achievement far too trivial to deserve counting. It must read as COMIC EXAGGERATION, never as a fact about their life. See the rule on invented dates below.
...
7. FIT — right for this relationship and occasion. A card for a nan shouldn't sound like one for a mate.
