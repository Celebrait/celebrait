...
/** ── THE GUEST MAKER GATE (the doorway's door one → /make)
 *  "Generate free, sign in to keep or buy." No key: the public /make
 *  builder calls the same four engine handlers. Exposure is bounded two
...
⚠️ WORDPLAY HERE MAY NEVER PUN ON THE OCCASION ITSELF. Puns on Christmas/present/season/trim/wrap/serve are the cracker shelf — the most worn wordplay in the entire market, and the audit found them in seven of ten sets ("Christmas Has Presence", "Present tense", "beautifully seasoned", "Trim and proper", "SERVE THE SEASON" — all spent, all banned). A Christmas pun lands only when it puns on the MATERIAL — the ritual, the kit, the role ("oven time and over-time" is the shape: the cook's shift, not the season's name). THE ONE EXEMPTION IS THE GREETING below, which may fuse their world into the greeting itself when it genuinely sings — a fusion, not a lazy pun.
⚠️ THE GREETING: at least ONE card's front carries Merry Christmas in some form. PLAYED STRAIGHT IS THE DEFAULT — "Merry Christmas", set beautifully, carried by the card's world. The RIFF (fusing their thing or a rude word into the greeting itself) is the exception, not the habit: use it only when the fusion genuinely sings, because a rack where every card riffs the greeting is a rack of one joke. One card greets; the other two are free.
...
THE QUIET CARD: at Christmas, the whisper-presence card IS the quaint one — one of Britain's best-selling Christmas registers and this rack's scarcest (observed). Pale airy ground (snow-pale, duck-egg, oat, winter-morning blue), a few homely objects drawn with real air around them, and the line set small like a private note. Cosy, modern, light-filled — never dark, never rich; candlelit depth and saturated grounds belong to the mid and full cards. Every set's whisper card plays this register.
...
const OCCASION_MATCHERS: Array<[RegExp, string]> = [
  ...
  [/engage/i, 'engagement'],
  [/gender reveal|reveal party/i, 'genderreveal'],
  [/baby ?shower|\bshower\b/i, 'babyshower'],
  [/baby|christening|newborn|new arrival|expecting/i, 'baby'],
...
- art_direction: one sentence — the MOTIF and how it sits in the chosen format.
  ...
  Where the OCCASION has a world of its own, let it inflect this: its season, its light, one of its objects folded into the interest's own kit, or its number worked into the scene. Never a generic occasion prop (balloons, cake, a wrapped present, a tree) dropped next to the real subject. ${characters === 'objects'
    ...
    ⚠️⚠️ HARD OVERRIDE — IF THE SUBJECT IS A FICTIONAL PROPERTY (a film, show, book, game, or its world), THERE IS NO HUMAN FIGURE ON ANY OF THE THREE CARDS, whatever the character setting says. Not a named one, not a generic one, not a silhouette, not "a girl by the sea". Inside that world's costume, palette and setting, ANY figure reads as ITS character — and we would be printing and posting someone else's property. Observed: a Moana brief with figures enabled returned a child in the character's own outfit in front of a wave, unmistakably her. Animals are permitted ONLY where the animal is ordinary and unowned (an owl, a pig) and never that world's named companion. Objects, places and scenery only — WITH ONE NARROW LICENCE: generic, UNIDENTIFIABLE figures seen FROM BEHIND are permitted when the property's iconic grammar is an ensemble scene (a family on a sofa shot from the telly's point of view). The test: strip the caption, and the figures must read as ANY family/any people — no costume, no silhouette, no colour signature that names a character. If the property is carried by ONE character's look, this licence does not apply. AND THE WORDS MUST HOLD UP THEIR HALF: naming the property is the entire reason the artwork is restricted, so EVERY front names or unmistakably anchors it (the buyer typed one thing; all three cards show it), and the set mines THE SHOW'S OWN WORLD — its rituals, its framing, what loving it actually looks like — never the generic category it belongs to. Observed failure: a Royle Family brief returned three telly-in-general sofa cards with nothing of the show anywhere; restraint in the artwork became amnesia in the words, which is the opposite of the deal. ⚠️ AND THE FAN'S LANGUAGE IS OPEN (Aidan 2026-08-31: "this show has so many epic quotes... why are we not capturing"): a property's SHORT CATCHPHRASES — the few words the whole country knows — may be quoted or riffed. They are how fans actually speak, and this card is one fan talking to another. Three limits, all of them: SHORT (a catchphrase, never a passage of dialogue — a sentence of dialogue is someone's copyrighted writing, three famous words are not); FUSED (the phrase does work in THIS recipient's joke — the birthday, the habit, the roast — never pasted as decoration); and NEVER OFFICIAL (nothing that implies the property made or endorsed the card — no title lettering in the show's own style, no episode-title formats). Dancing around a famous phrase with 'still quoting the show' is the observed timid failure — a card ABOUT quoting that daren't quote is fandom with its tongue cut out. AND THIS IS A DEAL, NOT A PERMISSION: when the property has a catchphrase the whole country knows, ONE of the three cards is BUILT ON the riff — the phrase aimed at this recipient's birthday, habit or verdict — because that is the card the fan actually wants and the rack is missing. Permission changes nothing; the deal changes everything. ⚠️ AND THE HALF-SECOND LAW OUTRANKS THE FANDOM (Aidan: "will this land in half a second — these are CRITICAL bits"): the buyer knows the show — that is why they typed it — but they are reading at rack speed. So the riff draws from the property's MOST FAMOUS tier (the phrase or beat the whole country knows, never the deep cut only a superfan places), and the line lands instantly FOR someone who knows it — fan knowledge may deepen the joke, it never buys a second read. A long scene-reconstruction is a fan essay, not a card front.
...
⚠️ IDENTITY OBJECTS ARE COLOUR-LOCKED. When the brief names a team, club or brand with famous colours, anything that BELONGS to it — a scarf, a shirt, a flag, a programme — wears ITS true colours or does not appear at all. A United scarf in yellow and black is a factual error the recipient reads instantly, like getting their name wrong. When a card's palette wants a different family, it gets there by choosing objects that carry no allegiance (the coffee, the phone, the motorway, the weather) — variety routes AROUND identity, never through it.
...
11. THE FIRST READ — a stranger gets the line in ONE pass, read aloud at card-shop speed. If the reader must re-read to work out who is doing what to whom, it FAILS however smart. Observed: "Kerry plans for the lights like the weather might need convincing" — grammatical, a real claim underneath, and a real buyer's verdict was "this does not make sense". Compression is never worth a second read: where the shortlist holds a clever-dense candidate and a plain-clear one, the plain one wins. This floor is floor 10 inverted — one kills lines that say nothing, this kills lines that say something too tightly to be received. TWO MORE OBSERVED SHAPES THAT FAIL IT: the TELEGRAM — staccato fragments the reader must reconstruct into a scene ("Fridge: absolutely not. Freezer: for f***'s sake, squeeze it in." — a real buyer's verdict on the set was "they simply do not make much sense"); and the BORROWED TERM — a slang word imported for its sound that does not fit this person ("Trolley dolly? Hardly. This is list-and-order." — trolley dolly is cabin-crew slang wearing a 60-year-old man's card, and the pun underneath needs excavating). If the line's key term would make the recipient say "what's that got to do with me?", it fails here. AND A THIRD SHAPE: the INVENTED COMPOUND — a coined noun phrase that does not resolve to a picture ("Next stop: Christmas hallway air and carrier-bag royalty." — a real buyer's response was "What does this mean??"). A coinage earns its place only when the reader sees what it names in ONE beat: "hallway air" nearly survives because you can smell it; "carrier-bag royalty" hands the reader two objects and an abstract noun and asks THEM to build the joke. If a compound needs a translator, the line wanted a plain verb instead. So does the GARDEN-PATH PUN: idiom-substitution wordplay that breaks the literal sentence ("spot a fake tortilla aisle away" — the missing "an" turns the pun into a parse error; the fix is usually one small word). ("December, and everyone's suddenly got a night version" is the same failure in deadpan clothing — night version OF WHAT is the reader's homework.) ⚠️ AND THE TRAP IS THAT YOU CAN DECODE THEM: you are a careful reader with unlimited time, and every one of these compounds will yield to you. The buyer gives the card ONE second in a crowded rack. Judge at THEIR speed, not yours — if the meaning arrives on a beat's delay even for you, it never arrives for them. AND A FOURTH: the UNANCHORED SCENARIO — a joke about a situation the card never names ("Merry Christmas — and good luck getting everyone out on time." — out of where? on time for what? A real buyer: "Wtf does the right one mean"). A joke may assume a scene ONLY when the words or the described artwork establish that scene; if the reader must supply the situation themselves, the joke is a lock without its key.
12. THE SWEAR-STRIP — rude fronts only. Delete the swearing and read what remains. It must still be a joke with a nameable engine: a dig at who they are, a double reading, an unsayable truth said flat, or filth in a formal register. Observed failure: the fridge/freezer card above — stripped, it is a food-storage instruction. A plain statement, instruction or compliment wearing a swear is not rude, it is decorated, and it FAILS however loud it reads.
...
- scene: ONE plain sentence stating who is doing what, where, according to the chosen line (plus its described artwork) — the floors 10 and 11 test made visible. Write it from the card alone, as a stranger would. If your sentence needs context the card never gives ("someone is getting people out of somewhere in time for something"), the line has FAILED those floors — choose another candidate or fix.
...
const wordsToRe = (ws: unknown): RegExp | null => {
  ...
  // A plural token must also catch its singular: the brief says
  // "lemons", the art direction says "lemon slices", and a substring
  // match calls that generic-artwork (observed false positive on a
  // set drowning in lemons). Trailing single-s becomes optional.
  return new RegExp(list.map((w) => w.replace(/[.*+?^${}()|[\]\\]/g, '\\$&').replace(/([^s])s$/i, '$1s?')).join('|'), 'i');
};

/** "No two cards share a leading hue" was a register rule holding at
 *  prompt-strength (~85%) until an 18-her set came back lilac, lilac,
 *  purple — the her-lean reaching for digital lavender as a uniform.
 *  Locked rules get code floors. The LEAD is the first chromatic
 *  colour named in the palette (ground first, by contract); neutrals
 *  (cream, stone, black...) are skipped so pale grounds stay legal.
 *  Family-level matching, or lilac vs lavender vs violet sails past. */
const COLOUR_FAMILIES: Array<[string, RegExp]> = [
  ['purple', /\b(purple|violet|lilac|lavender|mauve|plum|amethyst|grape|aubergine|heather|periwinkle)\b/i],
  ['pink', /\b(pink|rose|blush|fuchsia|magenta|bubblegum)\b/i],
  ['red', /\b(red|crimson|scarlet|cherry|burgundy|maroon|wine|claret|vermilion)\b/i],
  ['orange', /\b(orange|coral|apricot|peach|tangerine|amber|marigold|rust|terracotta)\b/i],
  ['yellow', /\b(yellow|citron|lemon|gold(?:en)?|mustard|butter(?:cup|milk)?|saffron|ochre)\b/i],
  ['green', /\b(green|olive|sage|mint|lime|emerald|forest|moss|pistachio|khaki)\b/i],
  ['teal', /\b(teal|turquoise|aqua|cyan|seafoam)\b/i],
  ['blue', /\b(blue|navy|cobalt|azure|cornflower|denim|indigo)\b/i],
  ['brown', /\b(brown|tan|chocolate|coffee|caramel|walnut|toffee)\b/i],
];
export function leadColourFamily(palette: string): string | null {
  let best: { fam: string; at: number } | null = null;
  for (const [fam, re] of COLOUR_FAMILIES) {
    const m = palette.match(re);
    if (m && m.index !== undefined && (!best || m.index < best.at)) best = { fam, at: m.index };
  }
...
export function v2Verify(cards: CardConcept[], b: V2Brief, hints: V2Hints, slots?: Array<{ register: string }>, opts?: { restingYear?: number; contextAge?: number; rudeSlot?: number; occasionKey?: string }): string[] {
  ...
  if (b.cheeky) {
    ...
    if (opts?.occasionKey === 'christmas') {
      // All three cards are rude; the grades differ. Hero = real swear
      // (enforced above/rude-slot); at least one MORE front carries a
      // mild; the third may be pure innuendo but never merely festive.
  ...
  // ⚠️ THE SWEAR-TAG FORMULA IS DEAD (2026-08-31). Observed twice
  // across subjects — "Birthday in New York? F***ing obviously." and
  // "Nobody gets a normal taco at yours. F***ing obviously." — a plain
  // claim with a two-word masked-swear sentence bolted after it. The
  // swear does no work inside the joke; it tags along. Convergence
  // across subjects = code.
  {
  ...
  if (opts?.occasionKey === 'christmas') {
    const GREET = /\b(merry|happy)\s+(?:[\w'’-]+\s+){0,2}(?:christmas|xmas|crimbo)\b|\b(?:merry|happy)\s+[\w'’-]*mas\b/i;
    if (!fronts.some((f) => GREET.test(f)))
      v.push('greeting-missing: not one front says Merry Christmas — at least one card carries the greeting, played straight by default, riffed only if the fusion genuinely sings');
  }
  ...
  // Aimed swears (nouns fired at the recipient) are directional; the
  // register explains, this floor guarantees. Peer relationships only,
  // and the direction must be known and match the word's home register.
  {
    const AIMED_HER = /\b(bitch(?:es)?)\b/i;
    const AIMED_HIM = /\b(bastards?|pricks?|twats?)\b/i;
    const PEERS = new Set(['sister', 'brother', 'best mate', 'friend', 'partner', 'anyone']);
    const who = b.who.trim().toLowerCase();
    const dir = b.gender !== 'unspecified' ? b.gender : who === 'sister' ? 'her' : who === 'brother' ? 'him' : null;
    cards.forEach((c, i) => {
      const t = `${c.front_text ?? ''} ${c.inside_text ?? ''}`;
      const her = t.match(AIMED_HER), him = t.match(AIMED_HIM);
      if (!her && !him) return;
      const w = (her ?? him)![0];
      if (!PEERS.has(who)) v.push(`aimed-swear: card ${i + 1} aims "${w}" at a ${b.who} — aimed swears are peer-only (sister, brother, best mate, friend, partner, or a rack card); rewrite on the intensifiers`);
      else if (!dir) v.push(`aimed-swear: card ${i + 1} uses "${w}" but the brief has no gender evidence — an aimed word must know who it is aimed at; rewrite on the intensifiers (f***ing, arse, bollocks)`);
      else if (her && dir !== 'her') v.push(`aimed-swear: card ${i + 1} aims "${w}" at a him — bitch is woman-to-woman conspiracy only; use the bloke-banter words or the intensifiers`);
      else if (him && dir !== 'him') v.push(`aimed-swear: card ${i + 1} aims "${w}" at a her — bastard/prick/twat are bloke-banter; use bitch (woman-to-woman) or the intensifiers`);
    });
  }
  {
    ...
    // one design decision made three times ("hot coral" observed in
    // every card of an 18-her set). Ownership excuses a thread only
    // when there IS a subject to own it — no interest, no exception.
    if (!b.interest) {
      for (const [fam, re] of COLOUR_FAMILIES) {
        if (cards.every((c) => re.test(String(c.palette ?? ''))))
          v.push(`colour-thread: the ${fam} family appears in all three palettes — one ink may not stitch the whole set together, and this brief names no subject to own it; drop it from at least one card and spread the world`);
      }
    }
  }
  ...
  if (hints.interest) {
    // ALL THREE (Aidan 2026-08-31: "if the user asks for something all
    // 3 cards should show it") — the buyer typed ONE thing; a card
    // that ignores it is a card they didn't ask for.
    whole.forEach((w, i) => {
      if (!hints.interest!.test(w)) v.push(`generic-set: card ${i + 1} never lands ${b.interest} in its words or artwork — the buyer asked for ONE thing and every card shows it`);
    });
  ...
  if (opts?.occasionKey && opts.occasionKey !== 'birthday') {
    ...
    if (re) {
      ...
      // FRONT + ARTWORK only: the inside is invisible on a rack, so a
      // Merry Christmas tucked inside cannot carry a front that reads
      // as any Friday night (observed: taxi-chips-chat works-do card,
      // zero Christmas on its face).
      cards.forEach((c, i) => {
        if (!(re.test(fronts[i]) || re.test(arts[i])))
          v.push(`occasion-missing: card ${i + 1} never lands ${opts.occasionKey} on its FACE — front words or artwork; the inside doesn't count because a rack browser never sees it. Right now this card could be any week of the year: fold the season into its own world (its light, one of its objects in the kit, or the words), never a stapled prop`);
      });
    }
  ...
  if (b.age) {
    ...
    const num = new RegExp(`\\b${b.age}\\b`);
    // Typeled exempt: its words ARE the artwork, so an art_direction
    // describing the numeral is the same appearance, not a second one
...
THE SUBJECT LENDS THE HUES; THE BRAND SETS THE FINISH. Take WHICH colours from their world — then print them the way a modern shop would, clean and alive, never faded down to seem tasteful. An allotment card is inked from the plot as printed today, not soil and sepia.
...
⚠️ DELICACY IS AN EQUAL CITIZEN OF THIS REGISTER — refinement, not the naive-charm voice. Fine linework, decorative detail worn lightly (a wreath, a botanical sprig, lace-fine pattern), painterly softness with air around it: these are as much the house look as the bold poster, and the TYPE WARDROBE is as wide — elegant high-contrast serifs, delicate romans and confident scripts belong in rotation alongside the heavy sans, every glyph still printing clean. And prize the GENTLE VISUAL METAPHOR as a concept shape: one quiet image standing for the whole thing, drawn finely, joke soft. The card a woman picks up first is as much ours as the card a designer photographs.
...
export function archetypeSystemPrompt(): string {
  return `You profile REAL PEOPLE for a UK card maker — the person, never the card tradition. ⚠️ Anything you know from old greeting cards is CONTAMINATION here, not insight: asked about a milestone, the tradition answers with symbols from decades-old cards (observed: ceremonial keys on a 21st) while the actual person answers with what their days genuinely contain NOW — profile the person alive in ${new Date().getFullYear()}, whatever their age. Return JSON {"archetype":"100-140 words. FIRST commit to the most likely SPECIFIC TYPE this person is on the brief's evidence — not the demographic average. Every interest splits into distinct tribes at EVERY age (the competitive one, the ritualist, the kit-obsessed one, the quietly devoted one, the social one...) — pick the likeliest for THIS person and profile THAT person. ⚠️ AN OBJECT-ONLY interest (lemons, candles, tractors) is an aesthetic, not a life — it hands the artist everything and the writer nothing. Profile the PERSON BEHIND THE OBJECT: what someone who loves this actually DOES — grows it, cooks with it, hosts around it, hunts it in markets — so every line has human behaviour to be about, never just a mood: the era they came of age in; what they ACTUALLY react to about this interest (famous layer + insider rituals); what reads cliché vs current to them (current is REGISTER and irony, never slang-stuffing — forced slang is the mum-trying failure); where the line is on cheek for this relationship","interest_words":["12-20 words/short phrases ONLY THIS EXACT SUBJECT owns — its places, people, eras, nicknames, rituals, slang. NEVER words that fit the broad category: for a football club, 'matchday' and 'team news' fit every club and prove nothing; 'Stretford End' proves everything"],"dislike_words":["same for the dislike, or empty"],"palette_world":"THE COLOUR WORLD THIS PERSON'S SUBJECT ACTUALLY LIVES IN, 8-16 words, theirs rather than a designer's default: the light it happens in, the materials it is made of, the hour they love it at. Then give it TODAY'S FINISH — name the hues as a modern print shop would ink them, clean and alive, never aged down to seem tasteful. ⚠️ THE AGE MUST NOT TINT THE COLOUR. No reasoning that older means muted, heritage or sepia, and none that younger means neon — every age shops in the same modern shop, and 'they are 60 so soften it' is the exact failure this field keeps producing. If the brief gave you NO subject, work from WHO IT IS FOR and the REGISTER — a 40-year-old sister and a 40-year-old brother should not receive interchangeable cards, and the recipient tilting the whole design's feel is exactly what a good shop does with its For Her and For Him walls. ⚠️ AND THE DEMOGRAPHIC NEVER CHOOSES THE HUE, IN EITHER DIRECTION. Pink-because-she's-a-woman, navy-because-he's-a-man, dodging pink to seem tasteful, lavender as youth shorthand — one failure, four costumes: all of them are colour argued from the PERSON'S CATEGORY instead of from the cards. Colour is chosen by WHAT THE CARDS WILL DEPICT: name this world's true colours and let every card paint what it actually shows in that thing's own colours. If the depicted thing has a colour, it gets ITS colour, however feminine, masculine, loud or soft that happens to be — and no subject was named here, so do not import one from these instructions. Name a ground and a leading colour, and never default to primary poster colour just because it is bold.","style_world":"THE VISUAL CULTURE THIS PERSON LIVES IN, 10-18 words — what is actually on their wall, their feed, their phone case, their shelves in ${new Date().getFullYear()}: the type, illustration and object idioms their world uses. This drives the MEDIUM and TYPE of the cards, especially when no interest is given (a subject brings its own visual history; a person alone must bring theirs). Be a SPECIFIC current tribe, never a demographic average, and never a crude stereotype — the test is whether this exact person would screenshot the card, not whether their category would. ⚠️ AND CURRENT MEANS CURRENT TO THEM. Every age has a 2026 visual culture of its own, and borrowing the young end's idiom (phone-UI closeups, app grids, screenshot-era posters) for an older recipient is the same failure as sepia on a 21st, just pointed the other way — observed: a 70-year-old's card built around a calendar-app screen. Her current world is real and specific; find HERS. ⚠️ BUT THE MAINSTREAM TRIBE IS A LEGITIMATE SPECIFIC ANSWER: the most popular taste of a cohort is still a real taste, and picking it when it is the likeliest truth is aim, not stereotype — do not dodge the obvious tribe just to seem original. What separates aim from stereotype is that you CHOSE this tribe on the evidence, not that the tribe is unusual.","territories":["EXACTLY THREE, one per card, and this is the field that stops a set being one joke told three ways. Each must come from a DIFFERENT REGION OF THIS PERSON'S LIFE — if two of them could come up in the same conversation, replace one. ⚠️ Your FIRST instinct for this brief is the region everyone reaches for; keep it as one of the three at most, and go genuinely looking for the other two. ⚠️ EXCEPTION — for milestone birthdays 18 to 25 the first-instinct region (permissions, admin, proof-of-age, the civic checklist of what is newly allowed) is SPENT: it may not be a territory at all, even once. Everything those years are actually spent doing is open — what they do that nobody sees, what they are like with other people, what they actually spend their time and money on, what has changed for them lately. Name each in 3-8 words, as an AREA to explore and never as a joke or a line."]}. Concrete, ${new Date().getFullYear()}, UK.`;
}

/** RUDE runs on four distinct comedy engines (Aidan 2026-08-30: "is
 *  rude just the same swearing over and over... why no rude jokes?").
 *  The floors graded swear-presence because swearing is regexable, so
 *  the writer learned that decent-line-plus-mask passes — Goodhart on
 *  our own floors. The engine is now DEALT per slot like every other
 *  piece of structure. Categories only, no example jokes (law 1). */
export const RUDE_MECHS: Record<string, string> = {
  roast: 'THE ROAST — affectionate brutality about who they actually are: the joke is a true, specific dig at the habit the brief implies, said with love the way close mates actually speak. The swearing seasons the dig; the DIG is the card.',
  innuendo: 'THE INNUENDO — the seaside double-entendre: a line innocent on its surface and filthy underneath, built from this world\'s OWN vocabulary (its objects and verbs must genuinely carry the second reading). This card may be the set\'s swear-light one — a mild word in full at most; the double reading does the filth.',
  truth: 'THE TOO-HONEST TRUTH — the thing everyone thinks about this occasion or this person\'s situation and nobody prints, said flat. The laugh is recognition; the swear is the exhale that comes with finally saying it.',
  formal: 'MISPLACED FORMALITY — obscenity delivered deadpan in entirely the wrong register: officialese, ceremony, small print, a polite notice, a listing. The collision between the register and the filth is the whole joke, so the formal voice must be played completely straight.',
};

export function v2SystemPrompt(visual: 'celebrait' | 'open' | 'charm', slots: Array<{ angle: string; format?: string; register: string; territory?: string; ground?: string; tone?: string; cast?: boolean; rudeMech?: string }>, occasionBrief: string): string {
...
PRESENCE does not choose the hue — each card's palette starts from what THAT CARD DEPICTS: the objects, place and materials drawn get their true colours, and the set spreads across the world's range from there.
...
- If the register is rude: real swearing is REQUIRED, and the joke must still survive with it removed. Distribution: by default at least two fronts carry it — but when a slot above is marked TONE=RUDE outside one-of-each, the swearing CONCENTRATES there (that card is the set's filthy hero and MUST carry one of the three real words on its front; the occasion brief explains the shape). A rude set with no real swearing anywhere has failed its entire register, whatever the occasion (the one exception: a slot dealt the INNUENDO engine may be the set's swear-light card — its filth is the double reading). ⚠️ WHERE A SLOT NAMES A RUDE ENGINE, BUILD THE JOKE ON THAT ENGINE — three rude cards running the same engine is one card at three volumes, and swearing is never the engine: every rude front must survive the SWEAR-STRIP (delete the swearing; what remains must still be a joke someone could laugh at — a dig, a double reading, an unsayable truth said flat, a register collision). If what remains is a plain statement or an instruction, the swear was doing all the work and the card is not rude, it is decorated. ⚠️ THE BOLTED TAG IS THE COMMONEST DISGUISE OF THIS FAILURE: a clean claim followed by a two-word swear sentence — "F***ing obviously.", "F***ing classic." — has now shipped on two unrelated subjects and is BANNED as a shape. The swear lives inside the sentence that carries the joke, changing its meaning, or it does not appear. ⚠️ MASKING HAS EXACTLY THREE WORDS. Only the f-word, the s-word and the c-word are ever masked (first letter, then asterisks). EVERYTHING ELSE PRINTS IN FULL, always: hell, damn, bloody, bitch, bastard, bollocks, arse, prick, twat and their kin are not maskable, and asterisks on any of them is an instant fail — observed twice in one set ("Son of a B****", "hush the h*** up"), both counterfeit rudeness that reads as a misprint. If a line leans on hell or damn to feel rude, it wanted one of the real three or a better joke. ⚠️ AND NO SWEAR WORD APPEARS ON TWO FRONTS OF ONE SET — a rack sells these side by side, and the same word twice reads as a tic, not a register (observed: bollocks on two of three). Three rude cards draw from three different corners of the vocabulary.
...
⚠️ READ WHETHER THIS IS A LOVE OR A PLAN. People type UPCOMING things into this box — a trip, a move, a new job — and an EVENT reads FORWARD: the excitement ahead, the world they are about to walk into. Claim NO history with it: no "still", no "always", no "another year of" unless the brief itself says they have done it before. Telling a first-time visitor they "still pick New York" is a printed factual error. A lifelong love may look back; a plan may only look forward.`
    ...
    if (body.pipeline !== 'classic' && !serious) {
      ...
      try {
        ...
        // The cast is DEALT, not permitted — the audit found zero cast
        // in thirty cards under permission alone. Kids always get a
        // cast card (matching the floor); adults roughly one set in
        // three, so Santa-at-the-bar exists without becoming wallpaper.
        const kidsBrief = statedAgeValue !== null && statedAgeValue <= 12;
        const xmasCast = occKey === 'christmas' && (kidsBrief || Math.random() < 0.34)
          ? Math.floor(Math.random() * 3) : null;
        // The rude ENGINE is dealt like every other piece of structure:
        // an all-rude set draws three different engines; one-of-each
        ...
        const slots = v2Angles.map((a, i) => ({ angle: a,
          cast: xmasCast === i || undefined,
          rudeMech: rudeMechFor(i),
        ...
        const userMsg = `${briefLines.join('\n')}\n\nWHO THIS PERSON IS (aim everything with this):\n${arch.archetype ?? ''}${
          typeof arch.palette_world === 'string' && arch.palette_world.trim()
            ...
            : ''}${
          typeof arch.style_world === 'string' && arch.style_world.trim()
            ? `\n\nTHE AESTHETIC WORLD THIS PERSON LIVES IN (the medium, type and motifs come from here — this is what they would actually put on a wall or screenshot, and it outranks house habit):\n${arch.style_world.trim()}`
            : ''}`;

        // 3. GENERATE → 4. REFEREE with one named-violation repair round.
        let concepts: CardConcept[] = [];
        let violations: string[] = [];
        // TWO repair rounds (2026-08-31). The weekend added detectors
        // (sense referee, anchors, swear floors) onto a single repair
        // pass, so honest flags shipped standing — "why so many things
...
⚠️ Your previous rewrite still contained protected material — remove it completely this time.` : ''}` },
  ...
  const renderHandler = async (req: Request, res: Response) => {
    ...
    const redrawPrompt = [
      ...
      body.cameoPhoto
        ? `ILLUSTRATION: THE PEOPLE FROM THE REFERENCE PHOTO as this card's stars — faces faithful to the photo at portrait scale, dressed for this world and genuinely taking part in it: ${body.art_direction}`
        : `ILLUSTRATION: ${body.art_direction}`,
      named.length
