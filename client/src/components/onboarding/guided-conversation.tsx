  ...
  const filteredSteps = useMemo(() => steps.filter(step => {
    ...
    if (streamlinedFlow) {
      ...
      if (streamlinedPhotoOption === 'upload_and_scene') {
        const allowedSteps = ['name', 'celebration', 'photo_upload', 'scene', 'art_style', 'message', 'inside_message', 'final_summary'];
                        ...
                        <div className="space-y-4">
                          <div className="space-y-2">
                            <Input
                              ...
                              className="text-xs md:text-sm p-4 min-h-[3rem] md:min-h-0 rounded-xl border-purple-200 focus:border-purple-400"
                              onKeyPress={(e) => e.key === 'Enter' && handleTextSubmit()}
                            />
                          </div>
                          <div className="flex justify-center">
