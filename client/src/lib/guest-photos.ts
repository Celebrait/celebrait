...
function ensureLoaded(): Promise<void> {
  ...
  if (!loading) {
    loading = idbAll().then((all) => {
      ...
      records = all.sort((a, b) => b.id - a.id).reverse();
      loaded = true;
      emit();
    });
  }
...
export async function guestUpload(args: GuestPhotoBlob): Promise<Photo> {
  await ensureLoaded();
  const id = nextId--;
  const src = args.imageBase64.startsWith('data:') ? args.imageBase64 : `data:image/jpeg;base64,${args.imageBase64}`;
  ...
  void (async () => {
    ...
    try {
      ...
      if (r.ok) {
        ...
        patch = {
          personCount: j.personCount ?? null,
          visualSummary: j.visualSummary ?? null,
          likeness: j.likeness ?? null,
          analyzedAt: new Date(j.analyzedAt ?? Date.now()),
        };
      }
    } catch { /* fail open */ }
